...
def configure_logging(level: int | str = logging.DEBUG) -> None:
    """Configure root logging so debug statements are emitted in script mode."""
...
def fit_isochrone_section_to_targets(
    ...
    if interpolator is None:
        n_points = int(work[["iso_color", "iso_mag"]].dropna().shape[0])
        n_unique_colors = int(work["iso_color"].dropna().nunique())
        logger.warning(
            "Skipping isochrone section (insufficient points for interpolation): "
            "logAge=%.3f, [M/H]=%.3f, n_points=%d, n_unique_colors=%d",
            age_log10_yr,
            metallicity_dex,
            n_points,
            n_unique_colors,
    ...
    logger.debug(
        "Prepared evaluation table for logAge=%.3f [M/H]=%.3f: usable_targets=%d of total_targets=%d",
        age_log10_yr,
        metallicity_dex,
        int(eval_df.shape[0]),
        int(targets.shape[0]),
    ...
    n_pred_nan = int(eval_df["iso_mag_pred"].isna().sum())
    if n_pred_nan:
        logger.debug(
            "Predicted magnitudes contain %d NaNs for logAge=%.3f [M/H]=%.3f (likely out-of-range colors)",
            n_pred_nan,
            age_log10_yr,
            metallicity_dex,
        )
...
def fit_spot_grid_to_targets(
    ...
    logger.info("Starting SPOT grid fit: n_iso_files=%d, sigma_mag=%.3f", len(iso_files), sigma_mag)
    if not iso_files:
        logger.error("No SPOT isochrone files were provided to fit_spot_grid_to_targets")
    ...
    for iso_file in iso_files:
        ...
        if not iso_path.exists():
            logger.error("SPOT isochrone file does not exist: %s", iso_file)
            continue
        ...
        for age, section_df in sections.items():
            ...
            try:
                ...
                n_value_errors += 1
                logger.warning(
                    "Failed fit for logAge=%s in %s due to invalid/missing columns: %s",
                    age,
                    iso_file,
                    exc,
                )
                logger.debug("Section columns for failed fit: %s", list(section_df.columns))
                continue
    ...
    if not results:
        logger.warning(
            "No valid SPOT fits were produced across all input files. "
            "n_sections_total=%d, n_value_errors=%d, target_columns=%s",
            n_sections_total,
            n_value_errors,
    ...
    n_finite = int(np.isfinite(results_df["log_likelihood"]).sum())
    logger.info(
        "Completed SPOT grid fit: n_sections_total=%d, n_value_errors=%d, "
        "n_results=%d, n_finite_logL=%d",
        n_sections_total,
        n_value_errors,
...
def test_fit_and_plot(
    ...
    if results_df.empty or best_eval_df.empty:
        raise RuntimeError("No valid SPOT fits were produced; inspect DEBUG logs for file paths, section columns, and interpolation diagnostics")

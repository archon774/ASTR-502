...
class SPOTIsochroneFitter:
    """Fit SPOT isochrone grid sections against CMD target data."""

    target_color_col = "BP_RP_abs"
    target_mag_col = "G_abs"

    def __init__(self, sigma_mag: float = 0.05):
    ...
    def fit_isochrone_section_to_targets(
        ...
        """Fit one isochrone section to targets and return scoring outputs."""
        logger.info(
        ...
        if interpolator is None:
            ...
            empty_cols = [self.target_color_col, self.target_mag_col, "iso_mag_pred", "mass_pred"]
            if host_col is not None:
                empty_cols.insert(0, "hostname")
            summary = LikelihoodSummary(0, float("-inf"), pd.Series(dtype=float))
            return result, summary, pd.DataFrame(columns=empty_cols)

        mask = targets[self.target_color_col].notna() & targets[self.target_mag_col].notna()
        eval_columns = [self.target_color_col, self.target_mag_col]
        ...
        eval_df["iso_mag_pred"] = interpolator(eval_df[self.target_color_col].to_numpy())

        ll_summary = dataframe_log_likelihood(
            observed=eval_df[self.target_mag_col],
    ...
    def fit_spot_grid_to_targets(
        ...
        for iso_file in iso_files:
            ...
            for age, section_df in sections.items():
                try:
                    fit, _, eval_df = self.fit_isochrone_section_to_targets(
                    ...
                    logger.warning("Failed fit for logAge=%s in %s", age, iso_file)
                    continue
    ...
    def _attach_mass_prediction(
        ...
        ref = track_df[["iso_color", mass_col]].dropna().sort_values("iso_color")
        if len(ref) < 2 or ref["iso_color"].nunique() < 2:
            eval_df["mass_pred"] = np.nan
            return
        ...
        eval_df["mass_pred"] = mass_interp(eval_df[self.target_color_col].to_numpy())


class FittingPlotter:
    """Plotting helpers for best-fit SPOT interpolation results."""
    ...
    def plot_fitted_model_against_targets(
        ...
        target_color_col, target_mag_col, pred_mag_col = "BP_RP_abs", "G_abs", "iso_mag_pred"
        ...
        ax.scatter(plot_targets[target_color_col], plot_targets[target_mag_col], s=8, alpha=0.45, color="black", label="Targets")
        ax.scatter(plot_fit[target_color_col], plot_fit[pred_mag_col], s=8, alpha=1, color="tab:red", label="Best logAge fit (per target)")
        ax.set_xlabel("BP-RP")
        ax.set_ylabel("G")
        ax.set_title(title)
        ax.grid(alpha=0.3)
...
class InterpolationWorkflow:
    """High-level workflow for loading data, fitting, saving outputs, and plotting."""

    def __init__(self, sigma_mag: float = 0.05):
        self.fitter = SPOTIsochroneFitter(sigma_mag=sigma_mag)
    ...
    def test_fit_and_plot(
        ...
        targets_df = TargetDataLoader.load_targets(phot_csv=phot_csv, dist_csv=dist_csv)
        results_df, best_eval_df, best_isochrone_df = self.fitter.fit_spot_grid_to_targets(
            targets_df=targets_df,
        ...
        save_csv = ResultsManager.save_best_fit_candidates(best_eval_df=best_eval_df, output_dir="results")
        resolved_save_path = Path(save_path) if save_path is not None else ResultsManager.default_plot_save_path("figs")
        fig_ax = FittingPlotter.plot_fitted_model_against_targets(
            targets_df=targets_df,
...
if __name__ == "__main__":
    LoggingUtils.configure_debug_logging("logs")
    workflow = InterpolationWorkflow(sigma_mag=0.05)
    workflow.test_fit_and_plot(

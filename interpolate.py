...
def _find_col(df: pd.DataFrame, candidates: Iterable[str]) -> str | None:
    ...
    norm_cols = {_normalize_col_name(c): c for c in cols}
    ...
    # 2) fallback: substring match on normalized names
    for cand in candidates:
        cand_norm = _normalize_col_name(cand)
        for col in cols:
            if cand_norm and cand_norm in _normalize_col_name(col):
                return col
    return None


def _find_target_name_col(df: pd.DataFrame) -> str | None:
    return _find_col(df, ["hostname", "host_name", "target", "name"])
...
def _prepare_isochrone_track(isochrone_df: pd.DataFrame) -> tuple[pd.DataFrame, str | None]:
    ...
    iso_g_col = _find_col(iso, ["gaia_g", "gmag", "g"])
...
def fit_isochrone_section_to_targets(
    ...
    target_name_col = _find_target_name_col(targets_df)

    targets[target_color_col] = pd.to_numeric(targets[target_color_col], errors="coerce")
    targets[target_mag_col] = pd.to_numeric(targets[target_mag_col], errors="coerce")
    ...
    if interpolator is None:
        ...
        empty_cols = [target_color_col, target_mag_col, "iso_mag_pred", "mass_pred"]
        if target_name_col:
            empty_cols.insert(0, target_name_col)
    ...
    eval_cols = [target_color_col, target_mag_col]
    if target_name_col and target_name_col in targets.columns:
        eval_cols = [target_name_col] + eval_cols
    eval_df = targets.loc[mask, eval_cols].copy()
